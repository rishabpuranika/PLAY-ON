...
export default function MangaLibrary() {
    ...
    const lists: LibraryList[] = useMemo(() => {
    ...
    useMemo(() => {
        if (lists.length > 0 && !activeTab) {
            // Prefer "Reading" or first available
            const reading = lists.find(l => l.name === 'Reading' || l.name === 'Current');
            setActiveTab(reading ? reading.name : lists[0].name);
        }
    }, [lists, activeTab]);
    ...
    const filteredAndSortedEntries = useMemo(() => {
        ...
        if (settings.filter.unread) {
            entries = entries.filter(e => {
                const total = e.media.chapters || 0;
        ...
        // "Downloaded", "Bookmarked", "Tracked" are placeholders for now as API doesn't fully support them or local logic is needed
    ...
    const gridColsClass = useMemo(() => {
        // Map itemsPerRow (2-8) to tailwind grid classes roughly
        // Since tailwind static scanning needs full class names, we'll try dynamic style or a switch
        // For simplicity, let's just stick to style attribute for generic counting
        return `grid gap-3`;
    }, []);
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <button
                                ...
                                className={`transition-colors ${
                                    // Highlight if any non-default filter/sort active?
                                    // For now just white/70
                ...
                >
                    {filteredAndSortedEntries.map(entry => (
                        <div
                            ...
                            onClick={() => navigate(`/manga-details/${entry.media.id}`)}
                        >
                            ...
                            <div className={`${settings.display.mode === 'List' ? 'flex flex-col justify-center' : 'mt-1.5'}`}>
                                ...
                                {settings.display.mode === 'List' && (
                                    <div className="text-xs text-white/50 mt-1">
                                        {entry.media.format} • Ch. {entry.progress}
                                    </div>
                                )}
                            </div>
                        </div>
